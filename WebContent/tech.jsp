...
//文档加载完后，获取所有动态信息
    $(function(){
    	//加载完后，要执行的代码就写在这匿名函数
    	$.get("http://localhost:8080/SCF/allCategorytech",function(result){
    		//document.write(result);
    		//遍历result（json数组）
    		for(var i=0;i<result.length;i++){
        		$("#ct").append("<li><span class='glyphicon glyphicon-hand-right'></span><a href='#'>"+result[i].atitle+"</a><span class='pull-right time'>"+'【'+result[i].adate+'】'+"</span></li>")
        	} 
        },"json")
    })
</script>
				...
				<div class="content_right pull-left col-lg-9">
					<h3>
						韶关市计算机学会<span class="pull-right">当前位置：<a href="/index.php">首页</a>
							&gt;&gt; <a href="/jishujiaoliu.html" title="技术交流" target="_top">技术交流</a></span>
					</h3>
					<ul class="news" id="ct">
						...
						<li><span class="glyphicon glyphicon-hand-right"></span><a
							href="jishujiaoliu.jsp">Apache提示You don't have
								permission to access / on this server</a><span
							class="pull-right time">【2015-12-27】</span></li>
					</ul>
					<div class="page pull-right"></div>
				</div>
			</div>
		</div>
</div>
