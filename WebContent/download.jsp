...
//文档加载完后，获取所有动态信息
    $(function(){
    	//加载完后，要执行的代码就写在这匿名函数
    	$.get("http://localhost:8080/SCF/SelectAllFile?curPage=1",function(result){

    		//document.write(result);
    		//遍历result（json数组）
    		for(var i=0;i<result.length;i++){
        		$("#dl").append("<div class='list_down_img pull-left'><img src="+result[i].dimage+" height='100' width='100'></div><div class='list_down_til pull-left'><p class='title'>"+result[i].dname+"</p><p class='time'>大小：<span>"+result[i].dsize+"</span>&nbsp;</p><p><a href='Download?filename="+result[i].ndocument+"' class='down'><u>点击下载</u></a>&nbsp;</p></div><div class='clearfix'></div><div class='con'><p>"+result[i].pdesc+"</p></div>")
        	} 
        },"json")
    })
</script>
				...
				<div class="content_right pull-left col-lg-9">
					<h3>
						韶关市计算机学会<span class="pull-right">当前位置：<a href="/index.php">首页</a>
							&gt;&gt; <a href="/xiazaizhongxin.html" title="下载中心"
							target="_top">下载中心</a></span>
					</h3>
					<ul class="news down" id="dl">
